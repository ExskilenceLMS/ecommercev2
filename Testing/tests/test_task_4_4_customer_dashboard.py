"""
Test cases for Task 4 - Subtask 4.4: Customer Dashboard UI
...
TEST_CASES_FILE = Path(__file__).parent / "test_cases" / "task_4_4_customer_dashboard.json"
...
@pytest.mark.e2e
class TestTask44CustomerDashboard:
    """End-to-end tests for Customer Dashboard UI wireframe"""
    ...
    def test_customer_dashboard_loads(self, page: Page, live_server):
        """Test customer dashboard loads and displays correctly"""
        test_case = next(tc for tc in self.test_cases if tc["name"] == "customer_dashboard_loads")
        page.goto(f"{live_server.url}{test_case['url']}")
        self._run_assertions(page, test_case["assertions"])
    
    def test_customer_dashboard_stat_cards(self, page: Page, live_server):
        """Test customer dashboard displays stat cards"""
        test_case = next(tc for tc in self.test_cases if tc["name"] == "customer_dashboard_stat_cards")
        page.goto(f"{live_server.url}{test_case['url']}")
        self._run_assertions(page, test_case["assertions"])
    
    def test_customer_dashboard_quick_actions(self, page: Page, live_server):
        """Test customer dashboard has quick action links"""
        test_case = next(tc for tc in self.test_cases if tc["name"] == "customer_dashboard_quick_actions")
        page.goto(f"{live_server.url}{test_case['url']}")
        self._run_assertions(page, test_case["assertions"])
